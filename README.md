...
The project has operating docs, source fixtures, parser/normalizer code, local Scryfall snapshot/index/search infrastructure, Search-2 filters, a browser deck library entry screen, a first frontend deck screen with visual-review polish, a tiny fixture-backed search/add workflow, browser-local native workspace open/save-as-download behavior, a factual local card-details panel, a compact mechanical validation panel, native `.mtgwdeck.json` workspace support, plain text import/export with CLI access, workspace mutation/category/annotation CLI access, read-only workspace view projections with local card-fact-backed type, mana-value, color, and color-identity support plus CLI access, deterministic role evidence, local card fact lookup, deck skeleton and structural warning reports, a bounded deck inspection envelope with CLI access, typed card relationship primitives, factual behavioral profiles, bounded behavioral atom extraction, deterministic relationship edge derivation, factual relationship reports, and stable end-to-end inspection, relationship, and workspace-view smoke fixtures.

No finished deckbuilder UI, recommendation engine, scoring rubric, structural audit engine, or full deck report exists yet. The current frontend is a browser-local deck library plus a fixture-backed deck screen with local UI-state search/add, factual card-details, compact mechanical validation behavior, and native JSON file picker/download support, not generic deckbuilder parity.

Current verified baseline: from `G:\Documents\New MTG project`, `python -m unittest discover -s tests` passes with 344 tests after Deck Library, Context Consolidation, and Save/Open UI v0.
...
Deckbuilder Foundation implementation includes the native workspace model, in-memory mutation helpers and CLI commands, category metadata and annotation CLI commands, read-only workspace view projections and CLI access, fact-backed type/mana-value/color/color-identity projection support with exact smoke-fixture coverage, plain text import/export and CLI commands, role-rule loading, local card fact lookup, card facts adaptation, card-level role evidence pipelines, structural deck reports, mechanical warnings, a factual deck inspection envelope and CLI command, typed relationship primitives, bounded behavior extraction, deterministic pairwise edge derivation, factual relationship reporting, stable smoke fixtures, a browser-local deck library, the first fixture-backed deck screen, local UI-state search/add behavior, browser file-picker open and save-as-download for native workspaces, a factual local card-details panel, and compact mechanical validation from current UI state. Do not add broad unapproved app UI, new frontend dependencies, full deck analysis, recommendations, scoring logic, strategic quality judgments, live APIs, or visual components until their own implementation slice is approved.

Frontend deckbuilder helper logic currently lives in `apps/deckbuilder-ui/src/deckUiLogic.ts`; `App.tsx` should stay focused on rendering and state orchestration. Completed UI checkpoints can be reviewed with the project-local `user-reviewer` skill and `docs/product/deckbuilder/USER_REVIEW_RUBRIC.md`, but final visual approval remains a human validation step. The first current-UI reviewer report is `docs/product/deckbuilder/reviews/CURRENT_UI_USER_REVIEW_2026-07-24.md`.
